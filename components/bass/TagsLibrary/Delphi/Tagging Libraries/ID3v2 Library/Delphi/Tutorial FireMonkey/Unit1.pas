...
begin
  ...
  begin
    ...
    try
      Success := ID3v2Tag.GetUnicodeCoverPictureStream(ID3v2Tag.FrameExists('APIC'), PictureStream, MIMEType,
...
begin
  ...
  begin
    ...
    // * Set it's data
    ID3v2Tag.SetUnicodeCoverPictureFromFile(FrameIndex, Description, OpenDialog1.FileName, MIMEType,
      PictureType);
  end;
end;
...
begin
  ...
  for i := ID3v2Tag.FrameCount - 1 downto 0 do
  begin
...
begin
  ...
  if ErrorCode <> ID3V1LIBRARY_SUCCESS then
  begin
...
begin
  ...
  begin
    MessageDlg('Error while removing ID3v1 tag, error code: ' + IntToStr(Error), TMsgDlgType.mtError,
      [TMsgDlgBtn.mbOk], 0);
    Exit;
  end;
  ID3v1Tag.Clear;
...
begin
  ...
  begin
    MessageDlg('Error loading ID3v2 tag, error code: ' + IntToStr(Error) + #13#10 +
      ID3v2TagErrorCode2String(Error), TMsgDlgType.mtError, [TMsgDlgBtn.mbOk], 0);
  end;
  ...
  if ID3v2Tag.MajorVersion = 3 then
  begin
...
begin
  ...
  if ErrorCode <> ID3V2LIBRARY_SUCCESS then
  begin
    MessageDlg('Error saving ID3v2 tag, error code: ' + IntToStr(ErrorCode) + #13#10 +
      ID3v2TagErrorCode2String(ErrorCode), TMsgDlgType.mtError, [TMsgDlgBtn.mbOk], 0);
  end;
...
begin
  ...
  if ErrorCode <> ID3V2LIBRARY_SUCCESS then
  begin
    MessageDlg('Error while removing ID3v2 tag, error code: ' + IntToStr(ErrorCode) + #13#10 +
      ID3v2TagErrorCode2String(ErrorCode), TMsgDlgType.mtError, [TMsgDlgBtn.mbOk], 0);
    Exit;
  end;
  ID3v2Tag.Clear;
...
begin
  // * Free the class
  if ID3v1Tag <> nil then
    FreeAndNil(ID3v1Tag);
  // * Free the class
  if ID3v2Tag <> nil then
    FreeAndNil(ID3v2Tag);
end;
...
begin
  ...
  try
    ...
    try
      ...
      Success := ID3v2Tag.GetUnicodeCoverPictureStream(Index, PictureStream, MIMEType, Description,
        PictureType);
  ...
  begin
    ...
    begin
      ...
      if (tempByte > 31) and (tempByte < 255) then
        asciiString := asciiString + chr(tempByte)
      else
...
begin
  if ID3v2Tag.Frames[ComboBox1.ItemIndex].Compressed then
  begin
  ...
  if ID3v2Tag.Frames[ComboBox1.ItemIndex].Unsynchronised then
  begin
