...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, Vcl.ComCtrls, TagsLibrary, JPEG, PNGImage, GIFImg, Vcl.ImgList;
...
begin
    ...
    Error := RemoveTagsFromFile(Edit1.Text);
...
begin
    //* Set specific tags
    Tags.SetTag('ARTIST', Edit2.Text);
    Tags.SetTag('TITLE', Edit3.Text);
    Tags.SetTag('ALBUM', Edit4.Text);
    //* Do the save
    Error := Tags.SaveToFile(Edit1.Text);
    if Error <> TAGSLIBRARY_SUCCESS then begin
        MessageDlg(TagsLibraryErrorCode2String(Error), mtWarning, [mbOk], 0);
    end;
    //* Re-load
    Button4Click(Sender);
end;
...
begin
    ...
    Tags.LoadFromFile(Edit1.Text);
    if Tags.Loaded then begin
        //* List specific tags
        Edit2.Text := Tags.GetTag('ARTIST');
        Edit3.Text := Tags.GetTag('TITLE');
        Edit4.Text := Tags.GetTag('ALBUM');
        //* List all tags
...
begin
    ...
    if FileExists(OpenDialog1.FileName) then begin
        try
            PictureStream := TFileStream.Create(OpenDialog1.FileName, fmOpenRead);
            try
                ...
                PictureStream.Seek(0, soBeginning);
                //* Add the cover art
                CoverArt := Tags.AddCoverArt(ExtractFileName(OpenDialog1.FileName));
...
begin
    ...
    Tags.DeleteCoverArt(ListView2.Selected.Index);
...
begin
    ...
    with Tags.CoverArts[ListView2.Selected.Index] do begin
        case Tags.CoverArts[ListView2.Selected.Index].PictureFormat of
            tpfJPEG: Fext := '.jpg';
            tpfPNG: Fext := '.png';
            tpfGIF: Fext := '.gif';
            tpfBMP: Fext := '.bmp';
        ...
        SaveDialog1.FileName := ChangeFileExt(Description, Fext);
        if SaveDialog1.Execute then begin
            TMemoryStream(Stream).SaveToFile(SaveDialog1.FileName);
        end;
    end;
end;
