...
begin
  ...
  begin
    ...
    addons.weather.Action.Yahoo.Data_Town[vTown_Num] := Get_Forecast(vTown_Num, vYahoo_Find_List[vNum].Woeid);

    vTemp_NewTown.Time_Results := Convert_Time(addons.weather.Action.Yahoo.Data_Town[vTown_Num].Observation.Time.TimeStamp,
      addons.weather.Action.Yahoo.Data_Town[vTown_Num].Observation.Time.WeekDay).Full;
    vTemp_NewTown.Forecast_Image := Get_Icon_From_Text(addons.weather.Action.Yahoo.Data_Town[vTown_Num].Observation.ConditionCode);
    vTemp_NewTown.Temperature := addons.weather.Action.Yahoo.Data_Town[vTown_Num].Observation.Tempreture.Now;
    vTemp_NewTown.Temrerature_Unit := Get_Unit(addons.weather.Action.Yahoo.Data_Town[vTown_Num].vUnit);
    vTemp_NewTown.Temperature_Description := addons.weather.Action.Yahoo.Data_Town[vTown_Num].Observation.ConditionDescription;
    vTemp_NewTown.City_Name := addons.weather.Action.Yahoo.Data_Town[vTown_Num].Location.City_Name;
    vTemp_NewTown.Country_Name := addons.weather.Action.Yahoo.Data_Town[vTown_Num].Location.Country_Name;
    vTemp_NewTown.Country_Flag := Get_Flag(addons.weather.Action.Yahoo.Data_Town[vTown_Num].Location.Country_Name);

    uWeather_Providers_Yahoo_Config.Towns_Add_New_Town(vTown_Num, vTemp_NewTown);

    uDB.vColumns := 'USER_ID, TOWN_NUM, TOWN_WOEID, TOWN_NAME';
    uDB.vValues := uDB_AUser.Local.Num.ToString + ', ' + vTown_Num.ToString + ', ' + vYahoo_Find_List[vNum].Woeid + ', ' + vTemp_NewTown.City_Name;

    Main_Create_Town(addons.weather.Action.Yahoo.Data_Town[vTown_Num], vTown_Num);
    if vTown_Num > 0 then
...
begin
  if uDB_AUser.Local.addons.Weather_D.Yahoo.Towns_Count <> -1 then
  begin
    for vk := 0 to uDB_AUser.Local.addons.Weather_D.Yahoo.Towns_Count do
    begin
