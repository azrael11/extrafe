...
begin
  ...
  vImgList := uWindows.File_Names(UDB_AUser.Local.ADDONS.Soundplayer_D.p_Images + '\playlist\', '*.png');
  vSoundplayer.scene.ImageList := TImageList.Create(vSoundplayer.scene.soundplayer);
  vSoundplayer.scene.ImageList.Name := 'A_SP_Playlist_Imagelist';

  for vi := 0 to vImgList.Count - 1 do
  begin
    vImage := TImage.Create(Main_Form);
    vImage.Width := 24;
    vImage.Height := 24;
    vImage.Bitmap.LoadFromFile(UDB_AUser.Local.ADDONS.Soundplayer_D.p_Images + '\playlist\' + vImgList[vi]);
    vImage.WrapMode := TImageWrapMode.Fit;
    vSoundplayer.scene.ImageList.Add(vImage.Bitmap);
    FreeAndNil(vImage);
  end;
...
begin
  ...
  vSoundplayer.Playlist.List.Images := vSoundplayer.scene.ImageList;
