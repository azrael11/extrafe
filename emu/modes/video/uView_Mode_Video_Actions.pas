  ...
  begin
    ...
    begin
      if FileExists(uDB_AUser.Local.EMULATORS.Arcade_D.Media.Icons + vList_Roms[ri] + '.ico') then
        Emu_VM_Video.Gamelist.Games.Line[vi].Icon.Bitmap.LoadFromFile(uDB_AUser.Local.EMULATORS.Arcade_D.Media.Icons + vList_Roms[ri]
          + '.ico');
...
begin
  { if FileExists(uDB_AUser.Local.EMULATORS.Arcade_D.Media.Videos + vList_Roms[vSelected] + '.mp4') then
    ...
    Emu_VM_Video.Media.Video.Game_Info.Favorite.Visible := uDB.Query_Select(uDB.Arcade_Query, 'fav_id_' + uDB_AUser.Local.Num.ToString, 'mame_status', 'romname',
    vList_Roms[vSelected]).ToBoolean; }
end;
