...
begin
  ...
  begin
    ...
    begin
      if extrafe.users_total > 0 then
      begin
        uDB_AUser.Online.Num := uDB_AUser.Find_User_Online_Num(uDB_AUser.Local.ID.ToString);
        uDB_AUser.Get_Online_Data(uDB_AUser.Online.Num.ToString);
        uDB.Query_Update_Online('USERS', 'ACTIVE', '1', uDB_AUser.Online.Num.ToString);
        uDB_AUser.Online.Active := 1;
        uDB.Query_Update(uDB.ExtraFE_Query_Local, 'USERS', 'ACTIVE_ONLINE', '1', 'USER_ID', '1');
        uDB_AUser.Local.Active := True;
      end;
    end;
  end;
