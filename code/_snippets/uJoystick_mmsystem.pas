...
type
  ...
  TJoyButton = (JoyBtn1, JoyBtn2, JoyBtn3, JoyBtn4, JoyBtn5, JoyBtn6, JoyBtn7, JoyBtn8, JoyBtn9, JoyBtn10, JoyBtn11, JoyBtn12, JoyBtn13, JoyBtn14, JoyBtn15,
    JoyBtn16, JoyBtn17, JoyBtn18, JoyBtn19, JoyBtn20, JoyBtn21, JoyBtn22, JoyBtn23, JoyBtn24, JoyBtn25, JoyBtn26, JoyBtn27, JoyBtn28, JoyBtn29, JoyBtn30,
    JoyBtn31, JoyBtn32);
  ...
  TJoyButtonEvent = procedure(Sender: TMMJoystick; const Buttons: TJoyButtons) of object;
  TJoyMoveEvent = procedure(Sender: TMMJoystick; const JoyPos: TJoyRelPos; const Buttons: TJoyButtons) of object;
  TJoyPOVChangedEvent = procedure(Sender: TMMJoystick; Degrees: Single) of object;
  ...
  EMMJoystickError = class(EComponentError);

  TMMJoystick = class(TComponent)
  private
    FActive: Boolean;
    FAdvanced: Boolean;
    FAxisCount: Byte;
  ...
  published
    property Joy_Name: String read FJoyName;
    property Manufacturer_Name: String read FMan_Id;
    property Product_Name: String read FPro_Id;
    property Reg_KEY: String read FReg_Key;
    property OEM_Name: String read FOEM_Name;
    property Joy_Connected: Boolean read FJoyIsConnected;
    ...
    property OnButtonDown: TJoyButtonEvent read FOnButtonDown write FOnButtonDown;
    ...
    property OnPOVChanged: TJoyPOVChangedEvent read FOnPOVChanged write FOnPOVChanged;
    property PollingInterval: Integer read GetInterval write SetInterval default 40;
    property RepeatButtonDelay: Cardinal read FRepeatButtonDelay write FRepeatButtonDelay default 350;
    property RepeatMoveDelay: Cardinal read FRepeatMoveDelay write FRepeatMoveDelay default 350;
    property RepeatPOVDelay: Cardinal read FRepeatPOVDelay write FRepeatPOVDelay default 350;
    property SuspendScreensaver: Boolean read FSuspendScreensaver write FSuspendScreensaver default False;
    property ThresholdFactor: Double read FThreshold write SetThreshold;
  end;

function Joystick: TMMJoystick;
function Joystick1: TMMJoystick;
function Joystick2: TMMJoystick;
...
procedure Register;
begin
  RegisterComponents('MM_Joy_Delphi', [TMMJoystick]);
end;

var
  FJoystick1: TMMJoystick = nil;
  FJoystick2: TMMJoystick = nil;
...
begin
  if FJoystick1 = nil then
    FJoystick1 := TMMJoystick.Create(nil);
  Result := FJoystick1;
end;
...
begin
  if FJoystick2 = nil then
    FJoystick2 := TMMJoystick.Create(nil);
...
resourcestring
  ...
  SErrNoJoystickAvail = 'Not enough joysticks available for another ' + 'TMMJoystick instance. Maximum joystick count is two.';

constructor TMMJoystick.Create(AOwner: TComponent);
begin
  if FJoystick1 = nil then
  begin
    FJoystick1 := Self;
  ...
  begin
    ...
    raise EMMJoystickError.Create(SErrNoJoystickAvail);
  inherited Create(AOwner);
  FInterval := 40;
  FRepeatButtonDelay := 5;
...
destructor TMMJoystick.Destroy;
begin
  SetActive(False);
...
procedure TMMJoystick.DoButtonDown(Buttons: Cardinal);
begin
...
procedure TMMJoystick.DoButtonUp(Buttons: Cardinal);
begin
...
procedure TMMJoystick.DoIdle;
begin
  if Assigned(FOnIdle) then
    FOnIdle(Self);
end;

procedure TMMJoystick.DoMove(const JoyPos: TJoyRelPos; Buttons: Cardinal);
begin
...
procedure TMMJoystick.DoPOVChanged(POV: Cardinal);
begin
...
function TMMJoystick.GetInterval: Integer;
begin
  if FAdvanced then
    Result := FInterval
  else
...
function TMMJoystick.Initialize(NeedAdvanced: Boolean = False): Boolean;
var
  JoyInfo: TJoyInfoEx;
  JoyCaps: TJoyCaps;
begin
  ...
  if (joyGetNumDevs <= FID) or (joyGetPosEx(FID, @JoyInfo) <> JOYERR_NOERROR) then
    Result := False
  else
  begin
    FJoyIsConnected := False;
    joyGetDevCaps(FID, @JoyCaps, SizeOf(JoyCaps));
    FJoyName := JoyCaps.szPname;
    FMan_Id := JoyCaps.wMid.ToString;
    FPro_Id := JoyCaps.wPid.ToString;
    FReg_Key := JoyCaps.szRegKey;
    FOEM_Name := JoyCaps.szOEMVxD;
    FAxisCount := Min(JoyCaps.wNumAxes, JoyCaps.wMaxAxes);
    FButtonCount := Min(JoyCaps.wNumButtons, JoyCaps.wMaxButtons);
    if (FAxisCount <>  0) and (FButtonCount <> 0) then
      FJoyIsConnected := True;
    FAxises := [axX, axY];
...
procedure TMMJoystick.InitTimer;
begin
  KillTimer(FWindowHandle, DefTimerID);
  if SetTimer(FWindowHandle, DefTimerID, FInterval, nil) = 0 then
    raise EMMJoystickError.Create(SErrNoTimersAvail);
end;

procedure TMMJoystick.ProcessAdvanced;
const
  JOY_RETURN = JOY_RETURNX or JOY_RETURNY or JOY_RETURNZ or JOY_RETURNR or JOY_RETURNU or JOY_RETURNV or JOY_RETURNPOVCTS or JOY_RETURNBUTTONS;
...
begin
  ...
  if joyGetPosEx(FID, @JoyInfo) = JOYERR_NOERROR then
    with JoyInfo do
    begin
      if (FPrevIdleTick = 0) and (LoWord(wXpos) = FCenter.X) and (LoWord(wYpos) = FCenter.Y) and (LoWord(wZpos) = FCenter.Z) and (LoWord(dwRpos) = FCenter.R)
        and (LoWord(dwUpos) = FCenter.U) and (LoWord(dwVpos) = FCenter.V) and (wButtons = 0) and ((not FHasPOV) or (dwPOV = JOY_POVCENTERED)) then
      begin
...
procedure TMMJoystick.ProcessSimple(var Message: TMMJoyMsg);
var
  JoyPos: TJoyRelPos;
begin
  with Message do
...
procedure TMMJoystick.SetActive(Value: Boolean);
begin
...
procedure TMMJoystick.SetAdvanced(Value: Boolean);
begin
...
procedure TMMJoystick.SetInterval(Value: Integer);
var
  JoyCaps: TJoyCaps;
begin
...
procedure TMMJoystick.SetThreshold(Value: Double);
var
  JoyThreshold: UINT;
begin
...
procedure TMMJoystick.WndProc(var Message: TMessage);
begin
