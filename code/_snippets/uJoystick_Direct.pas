...
//    destructor Destroy; override;
  ...
  TDXInputState = (isUp, isDown, isLeft, isRight, isButton1, isButton2, isButton3, isButton4, isButton5, isButton6, isButton7, isButton8, isButton9, isButton10,
    isButton11, isButton12, isButton13, isButton14, isButton15, isButton16, isButton17, isButton18, isButton19, isButton20, isButton21, isButton22, isButton23,
    isButton24, isButton25, isButton26, isButton27, isButton28, isButton29, isButton30, isButton31, isButton32);
...
begin
  ...
  begin
    ...
    begin
      ...
      if i = -1 then
        raise EForceFeedbackEffectError.CreateFmt('', [Name]);
      Effect := Effect[i];
...
procedure TForceFeedbackEffect.CreateEffect;
  ...
  function FindEffectCallBack(const pdei: TDIEffectInfoA; pvRef: Pointer): HRESULT; stdcall;
  begin
  ...
  procedure CreateIEffectGuid(const guid: TGUID; EffectObject: TForceFeedbackEffectObject);
  begin
    if EffectObject.Feff.dwSize = 0 then
      Exit;
  ...
  procedure CreateIEffect(dwFlags: DWORD; EffectObject: TForceFeedbackEffectObject);
  begin
    if EffectObject.Feff.dwSize = 0 then
      Exit;
    ...
    begin
      ...
      FRoot.FInput.FDevice2.EnumEffects(@FindEffectCallBack, Self, dwFlags);
...
begin
  ...
  if (FRoot.FInput = nil) or (FRoot.FInput.FDevice2 = nil) or (not FRoot.FInput.FForceFeedbackDevice) or (not FRoot.FInput.FForceFeedback) then
    Exit;

  if FEffectType = etNone then
    Exit;
...
begin
  ...
  begin
    ...
    dwFlags := DIEP_DIRECTION or DIEP_DURATION or DIEP_ENVELOPE or DIEP_GAIN or DIEP_SAMPLEPERIOD or DIEP_TRIGGERBUTTON or DIEP_TRIGGERREPEATINTERVAL or
    ...
    if FObject.FEffect <> nil then
      FObject.FEffect.SetParameters(FObject.Feff, dwFlags);
    if FObject2.FEffect <> nil then
      FObject2.FEffect.SetParameters(FObject2.Feff, dwFlags);
  end;
end;
...
begin
  ...
  begin
    if (Value = nil) or (FRoot <> Value.FRoot) then
      raise EForceFeedbackEffectError.CreateFmt('', ['Parent']);
...
begin
  ...
  begin
    FPlaying := False;
    if FObject.FEffect <> nil then
      FObject.FEffect.Stop;
    if FObject2.FEffect <> nil then
      FObject2.FEffect.Stop;
  end;
...
begin
  ...
  if FObject.FEffect <> nil then
    FObject.FEffect.Unload;
  if FObject2.FEffect <> nil then
    FObject2.FEffect.Unload;
...
function TCustomInput.SetDataFormat: Boolean;
  ...
  function DIEnumDeviceObjectsProc(const peff: TDIDeviceObjectInstanceA; pvRef: Pointer): HRESULT; stdcall;
  begin
    Result := Integer(DIENUM_CONTINUE);

    if CompareMem(@peff.guidType, @GUID_Unknown, SizeOf(TGUID)) then
      Exit;
...
begin
  ...
  begin
    ...
    if FDevice.SetDataFormat(FDataFormat) <> DI_OK then
      Exit;
  end;
  Result := True;
end;
...
begin
  ...
  if (not FEnabled) or (csDesigning in FDXInput.ComponentState) then
    Exit;

  if FDXInput.FDInput <> nil then
  begin
    if FDXInput.FDInput.CreateDevice(GUID_SysKeyboard, FDevice, nil) <> DI_OK then
      Exit;
...
begin
  ...
  if FDXInput.FDInput <> nil then
  begin
    if FDXInput.FDInput.CreateDevice(GUID_SysMouse, FDevice, nil) <> DI_OK then
      Exit;
    FDevice.SetDataFormat(c_dfDIMouse);
  end
  else
    raise EDXInputError.Create('');
...
//    Exit;
  ...
  if Fdims.lX < 0 then
    FStates := FStates + [isLeft];
  if Fdims.lX > 0 then
    FStates := FStates + [isRight];
  if Fdims.lY < 0 then
    FStates := FStates + [isUp];
  if Fdims.lY > 0 then
    FStates := FStates + [isDown];

  if Fdims.rgbButtons[0] and $80 <> 0 then
    FStates := FStates + [isButton1];
  if Fdims.rgbButtons[1] and $80 <> 0 then
    FStates := FStates + [isButton2];
  if Fdims.rgbButtons[2] and $80 <> 0 then
    FStates := FStates + [isButton3];
end;

{ TJoystick }

function SetDWORDProperty(pdev: IDirectInputDevice; guidProperty: pguid; dwObject, dwHow, dwValue: DWORD): HRESULT;
var
  dipdw: TDIPropDWORD;
begin
...
function SetRangeProperty(pdev: IDirectInputDevice; guidProperty: pguid; dwObject, dwHow, Value: DWORD): HRESULT;
var
  diprg: TDIPropRange;
begin
...
function TJoystick_EnumJoysticksCallback(const lpddi: TDIDeviceInstanceA; pvRef: Pointer): HRESULT; stdcall;
begin
  Result := Integer(DIENUM_CONTINUE);
...
begin
  ...
  if (not FEnabled) or (FID < 0) or (csDesigning in FDXInput.ComponentState) then
    Exit;
  ...
  try
    try
      ...
      begin
        ...
        FDXInput.FDInput.EnumDevices( { DIDEVTYPE_JOYSTICK } 4, @TJoystick_EnumJoysticksCallback, Self, DIEDFL_ATTACHEDONLY);

        if not FEnumFlag then
          Exit;

        { Device making. }
        if FDXInput.FDInput.CreateDevice(FDeviceGUID, FDevice, nil) <> DI_OK then
          Exit;
...
begin
  if (Obj < Low(FDeadZone)) or (Obj >= High(FDeadZone)) then
    Exit;
...
begin
  if (Obj < Low(FRange)) or (Obj >= High(FRange)) then
    Exit;
...
//    Exit;
  ...
  begin
    ...
    begin
      ...
      JoyInfo.dwFlags := JOY_RETURNX or JOY_RETURNY or JOY_RETURNZ or JOY_RETURNPOV or JOY_RETURNBUTTONS or JOY_RETURNCENTERED;
...
type
  TDirectInputCreate = function(hinst: THandle; dwVersion: DWORD; out ppDI:
{$IFDEF UNICODE}IDirectInputW{$ELSE}IDirectInputA{$ENDIF}; punkOuter: IUnknown): HRESULT; stdcall;
begin
  if FDirectInput = nil then
  begin
    try
      TDirectInputCreate(DXLoadLibrary('DInput.dll', 'DirectInputCreateA'))(HInstance, DIRECTINPUT_VERSION, FDirectInput, nil);
    except
      FDirectInput := nil;
    end;
  end;
...
begin
  ...
  begin
    if FUseDirectInput then
      InitDirectInput(FDInput);
