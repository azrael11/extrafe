...
begin
  ...
  case Message.Msg of
    CM_ACTIVATE:
      begin
        DoActivate;
        if FInitialized and FActiveOnly then
          Resume;
      end;
    CM_DEACTIVATE:
      begin
...
begin
  if Source is TDXPictureClip then
  begin
    with TDXPictureClip(Source) do
    begin
      Self.FRows := Rows;
      Self.FCols := Cols;
      Self.FMasked := Masked;
...
procedure TDXPictureClip.Draw(Canvas: TCanvas; X, Y, Index: Integer);
  ...
  procedure StretchBltTransparent(DstDC: HDC; DstX, DstY, DstW, DstH: Integer; SrcDC: HDC; SrcX, SrcY, SrcW, SrcH: Integer; Palette: HPalette;
  ...
  procedure StretchBitmapTransparent(Dest: TCanvas; Bitmap: TBitmap; TransparentColor: TColor; DstX, DstY, DstW, DstH, SrcX, SrcY, SrcW, SrcH: Integer);
  var
  ...
  begin
    ...
    try
      ...
      begin
        StretchBlt(Dest.Handle, DstX, DstY, DstW, DstH, Bitmap.Canvas.Handle, SrcX, SrcY, SrcW, SrcH, Dest.CopyMode);
      end
      else
      begin
        ...
        StretchBltTransparent(Dest.Handle, DstX, DstY, DstW, DstH, Bitmap.Canvas.Handle, SrcX, SrcY, SrcW, SrcH, Bitmap.Palette, TransparentColor);
      end;
    finally
  ...
  begin
    StretchBitmapTransparent(Dest, Bitmap, TransparentColor, DstX, DstY, Bitmap.Width, Bitmap.Height, 0, 0, Bitmap.Width, Bitmap.Height);
  end;
...
function TDXPictureClip.GetGraphicCell(Index: Integer): TBitmap;
  ...
  begin
    ...
    begin
      ...
      begin
        Dest.Canvas.CopyRect(Bounds(0, 0, CellWidth, CellHeight), TBitmap(Source).Canvas, Bounds((Index mod Cols) * CellWidth, (Index div Cols) * CellHeight,
      ...
      begin
        ...
        Dest.Canvas.Draw(-(Index mod Cols) * CellWidth, -(Index div Cols) * CellHeight, Source);
      end;
...
begin
  ...
  begin // 0-63
    if dY > 0 then
      Result := ArcTan(dY / dX) // 0 < Res < 90
    else // 0
      if dY = 0 then
        Result := 0 // 0
      else // 192-255
        if dY < 0 then
          Result := 2 * PI + ArcTan(dY / dX) // 270 < Res < 360
  end
  else if dX = 0 then
  begin // 64
    if dY > 0 then
  ...
  begin // 64-127
    if dY > 0 then
      Result := PI + ArcTan(dY / dX) // 90 < Res < 180
    else // 128
      if dY = 0 then
        Result := PI // 180
      else // 128-191
        if dY < 0 then
          Result := PI + ArcTan(dY / dX) // 180 < Res < 270
  end;
